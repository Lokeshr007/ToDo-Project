...
function Dashboard() {
  ...
  const fetchDashboardData = async () => {
    setLoading(true);
    try {
      console.log("Fetching dashboard data for workspace:", currentWorkspace);
      ...
      // Tasks by project
      const tasksByProject = projectsRes.data.map(project => ({
        name: project.name,
  ...
  const fetchRecentActivities = async () => {
    try {
      ...
      // Generate mock activities from recent tasks
      const mockActivities = allTodos.slice(0, 5).map((todo, index) => ({
        id: index,
  ...
  const todayTasks = allTodos.filter(t => t.dueDate && isToday(new Date(t.dueDate)) && t.status !== 'COMPLETED');
  const upcomingTasks = allTodos.filter(t => {
    if (!t.dueDate || t.status === 'COMPLETED') return false;
